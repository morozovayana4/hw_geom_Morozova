...
void add_edge_to_graph(vector<vector<int>>& g, int u, int v, int n) {
    if ( u == v) {
        return;
    }
    g[u].push_back(v);
...
bool is_gabriel_edge(const vector<Point>& points, int n, int u, int v) {
    for (int k = 0; k < n; ++k) {
        if (k != u && k != v) {
            long double x1 = points[u].x - points[k].x;
            long double y1 = points[u].y - points[k].y;
            long double x2 = points[v].x - points[k].x;
            long double y2 = points[v].y - points[k].y; 
            if (x1 * x2 + y1 * y2 <= EPS) {
                return false;
            }
        }
...
int main() {
 ...
 /*   bool all_collinear = true;
    ...
    for (auto& tdata : triangles) {
        if (tdata.alive) {
            if (tdata.a < n && tdata.b < n && tdata.c < n) {
                add_edge_to_graph(graph, tdata.a, tdata.b, n);
                add_edge_to_graph(graph, tdata.b, tdata.c, n);
                add_edge_to_graph(graph, tdata.c, tdata.a, n);
            }
        }
    }
    for (int i = 0; i < n; ++i) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j : graph[i]) {
            if (i < j) {
                if (is_gabriel_edge(points, n, i, j)) {
